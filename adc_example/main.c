/* 
* ADC: Sample voltage on GPIO P0.05 at 10 Hz and send to UART
 */
...
// Enable external 16MHz crystal oscillator
...
// Cofigure the PPI Channel
static void ppi_init(void)
{
  // Configure PPI channel 3 to start ADC task
  NRF_PPI->CH[3].EEP = (uint32_t)&NRF_RTC0->EVENTS_TICK;
...
void ADC_IRQHandler(void)
{
  ...
  /* Write ADC result to UART */
 	simple_uart_put(NRF_ADC->RESULT); 
}
...
int main(void)
{
  ...
  rtc_config();				//Configure RTC timer
	...
	simple_uart_config(0, TX_PIN_NUMBER, 0, RX_PIN_NUMBER, false);	//Configure UART
